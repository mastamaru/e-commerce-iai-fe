        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav>
            <ul className="flex space-x-4">
              ...
              <li><a href="#" className="text-gray-700 hover:text-gray-900 underline">Orders</a></li>
            </ul>
          </nav>
          <div className="flex items-center space-x-2">
            <span className="text-gray-700">Hi, Hiera</span>
            <img src="/hero.png" alt="User Avatar" className="w-8 h-8 rounded-full" />
          </div>
        </div>
      </header>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 tracking-wider">Items</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 tracking-wider">QTY</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 tracking-wider">Order Total</th>
                ...
                <tr key={order.orderId}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {order.items.map((item, index) => (
                      <div key={index}>{item.name} Size: {item.size} ({item.qty})</div>
                    ))}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">{order.items.reduce((acc, item) => acc + item.qty, 0)}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{order.orderTotal}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <button className="px-4 py-2 text-sm font-medium text-black border border-black bg-white-500">{order.status}</button>
                  </td>
                </tr>
              ))}
            </tbody>
